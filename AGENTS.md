...
- README download links are hardcoded per version (`Revision_X.Y.Z_*.dmg` etc.) — update them in README when bumping versions. Asset name patterns: `Revision_{ver}_aarch64.dmg`, `Revision_{ver}_x64.dmg`, `Revision_{ver}_x64-setup.exe`, `Revision_{ver}_x64_en-US.msi`.
